...
: <<'END_COMMENT'
This script performs the following actions on a Manjaro Docker instance:
...
To copy the executable from this Docker container to your local machine:
  ...
  3. Use docker cp to copy the executable:
     docker cp CONTAINER_ID:/DataSurgeon/target/release/ds-v${DS_VERSION}-manjaro-x86_64 /path/on/your/host/machine
END_COMMENT
DS_VERSION="1.2.4"
...
pacman -S --noconfirm curl wget git pkg-config openssl gcc make base-devel
...
mv target/release/ds target/release/ds-v${DS_VERSION}-manj
